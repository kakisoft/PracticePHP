...
現に、現在のプロジェクトでは、update の時に発生するデッドロックは、
現に、現在のプロジェクトでは、update 時に発生するデッドロックは、
...
プライマリキーを変えるというのは、結構コストの高い修正になるので、
プライマリキーを変えるというのは、結構コストの高い修正になるケースも多いので、
...
などの挿入文（一括挿入）時に対してAUTO_INCロックを取得する。
などの挿入文（一括挿入）時に対して AUTO_INC ロックを取得する。

一括挿入のAUTO_INCREMENTの順番が守られるので，
一括挿入の AUTO_INCREMENT の順番が守られるので，

すべての挿入する文に対してAUTOINCロックを取得しない。
すべての挿入する文に対して AUTOINC ロックを取得しない。

ただし，一括挿入のAUTO_INCREMENTの順番が守られないので、行ベースでのみレプリケーション可能となる。  
ただし、一括挿入の AUTO\_INCREMENT の順番が守られないので、行ベースでのみレプリケーション可能となる。  

「2」の場合、
「2：インターリーブ ロックモード」の場合、
