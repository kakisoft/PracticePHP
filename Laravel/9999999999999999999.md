...
Camel case will be used
Camel case is used  -> こっち。技術書は主語 You を使わない
...
URI design standards are as follows.


コントローラ名は単数形です。これは　Laravel ベストプラクティスのルールに則っています。
Controller name is singular. It's  following Laravel best practice.

役割
function

Method name in the Controller layer.
---------------------------------------------------------------------------------
...
今回のプロジェクトでは、バックエンドのフレームワークに Laravel を採用しました。
なので、言語はPHPとなります。
We have adapted Laravel in this project, so the Language is PHP.

また、今回のプロジェクトでは、コーディグ規約にて、PHPのメソッド名をキャメルケースとしています。
And, for this project, PHP method name is camel case in accordance with coding standards.


なので、今回提案している API設計ルールは、上記のパターンに特化している部分があります。
So, The API design rule that I suggest is partly specific to these pattern.


そのため、別の言語にて適用する場合には、一工夫する必要がありますが、それについても説明をしていきます。
...
------------------------------------------------------------
バックエンドの言語を Goに置き換えると、不自然な部分が出てきます。
If you replace the backend language with Go Lang, the API design rule will have unnatural part.

なぜなら、Go言語は、メソッド名をアッパーキャメルを使う事が多いからです。
Because, Go Lang usually use upper case for method name.

なので、URLの部分は、ケバブケースを使うという考えもアリでしょう。
